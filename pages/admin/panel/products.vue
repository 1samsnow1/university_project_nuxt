<template>
    <section>
        <div v-if="panelStatus=='list'" class="p-2 rounded-md min-w-[240px] sm:min-w-[400px] max-w-xl mx-auto bg-gray-100">
            ...
            <p v-if="newsList && newsList.length == 0" class="text-red-600 text-center">پروژه ای موجود نیست</p>
            <ul v-else class="flex flex-col gap-1">
                <li v-if="newsList" v-for="item in newsList" class="w-full flex items-center justify-between bg-gray-800 py-1 px-3 text-xs sm:text-sm md:text-base rounded-lg">
                    ...
                    <div class="w-full flex flex-col sm:flex-row items-start sm:items-center gap-4 ml-1 sm:ml-2">
                        <div class="">
                            <span class="text-gray-100 max-w-[260px] overflow-hidden overflow-ellipsis line-clamp-1">{{item.name}}</span>
                        </div>
                        <div>
        ...
        <AdminFormAdd v-if="panelStatus=='add'" :panelName="'products'" @changeForm="changeForm"/>
    </section>
</template>
...
const fetchNewsList = async ()=>{
    const { data } = await useFetch('https://resume.bargh-saman.ir/api/products');
...
const deleteProduct = async (id)=>{
    try {
        ...
        const deletep = await $fetch(`https://resume.bargh-saman.ir/api/ProductManagement/product/${id}`,{
            method:'DELETE',
            headers:{
